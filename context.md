...
- Corrected the icon-only navigation geometry from the user-supplied crop: removed the obsolete empty top label plate, expanded each dashed service box to 97 × 80 SVG units, and centered all four glyphs at x + 63.5/y 110. Icon proportions were normalized per symbol while button bounds, link targets, screws, lamps, and hover/press behavior remain unchanged.
- Deepened the four icon-only navigation keys into compact spacecraft console switches without changing the navigation module footprint. Each key now has an octagonal metal shell, raised chamfered cap, side guard rails, inset corner brackets, an under-cap shadow, and a recessed lamp housing. The complete cap travels 3 SVG pixels when pressed, while the existing icon, accessible link, and blue-to-yellow hover lamp behavior remain intact.
- Added two symmetric suspension struts behind the navigation module so the full header reads as ceiling-mounted spacecraft hardware. Each outlined metal bar runs from SVG y -120 into a bolted clamp behind the rail; during the 18px downward handoff bounce, browser measurements keep the strut tops 51.16px above the viewport, so no gap appears before the header slings upward.
- Moved the animated radar's current-position beacon from the route origin to the cubic path midpoint at x 220/y 105. The route now terminates at the destination center, while a circularly clipped second rendering of the existing `mission-radar.png` planet is layered above the animated dots so the route disappears naturally behind the planet instead of crossing its surface.
- Rebuilt the header's Ask Shadow control as a compact old television while retaining the same semantic button, keyboard handling, chat state, and footprint. The CRT uses the existing purple identity inside a brown-black casing with curved glass, scanlines, a reflection, twin antennae, rotary controls, speaker slots, and small feet; activating it still opens and closes the custom Shadow chat.
