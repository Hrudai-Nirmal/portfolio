...
- Replaced the rejected full-header raster shortcut with a responsive, interactive SVG reconstruction. The identity chassis, four linked navigation keys, Shadow command screen, cabling, indicators, screws, and hazard details are vector elements; the aligned yellow menu control remains a real animated input. The rail stays fixed instead of fading on scroll.
- Scaled the persistent desktop rail to 70%, removed the yellow markings from its red alert indicator, and made each navigation key's blue status light turn yellow on that key's hover state. At compact breakpoints, the menu now owns the top-right position in a higher stacking layer while Ask Shadow stays top-left.
