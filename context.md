...
- Replaced the desktop rail's embedded menu control with an AUX THRUST module. Crossing 160px of scroll now triggers an autonomous animation: the rail dips 18px before slinging above the viewport while the detached compact menu slings into the top-right. Crossing the same point upward reverses the timeline; neither element is scrubbed directly by scroll position. The menu remains fixed after the handoff, while compact layouts keep their always-visible menu behavior.
