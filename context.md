...
- Replaced the desktop rail's embedded menu control with an AUX THRUST module. Crossing 160px of scroll triggers an autonomous animation: the rail dips 18px before slinging above the viewport while the detached compact menu slings into the top-right. The 1.12-second handoff is 1.3× faster than the previous pass and uses sine/exponential easing for a controlled middle followed by a sharper launch and drop. Crossing the same point upward reverses the identical timeline; neither element is scrubbed directly by scroll position. The menu remains fixed afterward, while compact layouts keep their always-visible menu behavior.
